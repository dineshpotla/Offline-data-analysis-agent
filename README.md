...
- Install deps:
  ...
  ```
- Download the LFM2-2.6B model locally (offline). Place it in:
  ```
  ./models/LFM2-2.6B
  ```
  or set:
...
### Interactive chat (follow-ups remember context)
```bash
python agent_cli.py --chat --file data.csv
```
Use follow-ups like "now filter outliers" or "save report to outputs/report.txt".
...
- Custom code: planner can emit `run_python` steps to manipulate data or save files when requested.

## UI (Streamlit)
Run a local, offline web UI:
```bash
streamlit run ui_streamlit.py
```
Upload a dataset and chat in the browser.
