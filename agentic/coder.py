...
def generate_code(plan: Dict[str, Any]) -> str:
    ...
    for step in plan.get("steps", []):
        ...
        if action == "load_file":
            ...
            lines.append(f'result = eda.plotly_correlation_heatmap(df, path="{path}")')
        elif action == "plotly_distributions":
            path = args.get("path", "outputs/distributions.html")
            cols = args.get("cols")
            bins = args.get("bins", 30)
            if cols:
                lines.append(f"result = eda.plotly_distributions(df, cols={cols}, path='{path}', bins={bins})")
            else:
                lines.append(f"result = eda.plotly_distributions(df, path='{path}', bins={bins})")
